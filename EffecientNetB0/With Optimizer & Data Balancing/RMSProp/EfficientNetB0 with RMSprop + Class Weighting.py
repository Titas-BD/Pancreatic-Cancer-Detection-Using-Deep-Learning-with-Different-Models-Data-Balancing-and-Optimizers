...
def dicom_generator(paths, labels, batch_size, num_classes, datagen=None):
    """
    Yield batches of augmented (or plain) DICOM images and one-hot labels.
    """
    while True:
        x_batch, y_batch = [], []
...
# DATA AUGMENTATION (does not change class balance, only variability)
...
# CLASS WEIGHTS (data balancing by weighting loss per class)
...
# PREPARE VALIDATION DATA (for metrics and final evaluation)
val_images = []
val_labels_clean = []
...
optimizer = RMSprop()  # explicit RMSprop optimizer
