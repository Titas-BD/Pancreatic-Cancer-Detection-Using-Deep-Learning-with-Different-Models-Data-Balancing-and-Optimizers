...
# Dataset Path
DATASET_PATH = (
    r"E:\Work folder\AIUB\Reasearch Folder\Thesis_G42\PancreasProject"
    r"\manifest-1661266724052\Pancreatic-CT-CBCT-SEG"
)
...
# ==========================
# SMOTE+ENN on training images
# ==========================
print("[INFO] Loading training images into memory for SMOTE+ENN...")
train_images = []
train_labels_clean = []
...
print(f"Training samples before SMOTE+ENN: {train_images.shape[0]}")

n_samples, h, w, c = train_images.shape
X_flat = train_images.reshape(n_samples, -1)

smote_enn = SMOTEENN(random_state=SEED)
X_res_flat, y_res = smote_enn.fit_resample(X_flat, train_labels_clean)

X_res = X_res_flat.reshape(-1, IMAGE_SIZE, IMAGE_SIZE, 3)
y_res = np.array(y_res, dtype=np.int32)

print(f"Training samples after SMOTE+ENN: {X_res.shape[0]}")
...
# TRAIN (AdaBelief + SMOTE+ENN, no class_weight)
model.fit(
    train_gen,
...
print("\nResults (EfficientNetB0 + AdaBelief + SMOTE+ENN):")
