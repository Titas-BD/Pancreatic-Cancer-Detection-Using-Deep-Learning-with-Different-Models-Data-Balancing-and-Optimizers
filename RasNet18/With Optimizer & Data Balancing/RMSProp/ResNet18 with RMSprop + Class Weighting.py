...
def main():
    ...
    class_weights = {cls: w for cls, w in zip(np.unique(y_int), class_weights_arr)}
    ...
    # Train / test split (no separate val set, use validation_split in fit)
    ...
    # Optional: simple training curves
    plt.figure(figsize=(8,5))
    plt.plot(history.history['accuracy'], label='Train Acc')
    plt.plot(history.history['val_accuracy'], label='Val Acc')
    plt.title('ResNet18 Accuracy (RMSprop + Class Weights)')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    plt.legend()
    plt.show()
    ...
    plt.title('ResNet18 Loss (RMSprop + Class Weights)')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
    plt.show()
