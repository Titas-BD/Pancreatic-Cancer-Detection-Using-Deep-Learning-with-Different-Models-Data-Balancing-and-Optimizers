...
def main():
    ...
    # ==========================
    # SMOT: RandomOverSampler
    # ==========================
    idx = np.arange(len(y_int)).reshape(-1, 1)
    ros = RandomOverSampler(random_state=random_state)
    idx_resampled, y_resampled = ros.fit_resample(idx, y_int)

    idx_resampled = idx_resampled.ravel()
    images_resampled = images[idx_resampled]
    y_int_resampled = y_resampled

    print(f"Original dataset size: {images.shape[0]}")
    print(f"Resampled dataset size (SMOT): {images_resampled.shape[0]}")
    ...
    X_train, X_test, y_train, y_test, y_train_int, y_test_int = train_test_split(
        images_resampled, y_resampled, y_int_resampled,
        test_size=test_size,
        random_state=random_state,
        stratify=y_int_resampled
    )

    del images, labels, images_resampled, y_resampled
    gc.collect()

    # Build and compile ResNet18 with RMSprop
    ...
    # Train (no class_weight here, balancing is handled by SMOT oversampling)
    history = model.fit(
        X_train, y_train,
        epochs=epochs,
    ...
    plt.title('ResNet18 Accuracy (RMSprop + SMOT)')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    plt.legend()
    plt.show()
    ...
    plt.title('ResNet18 Loss (RMSprop + SMOT)')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
    plt.show()
